...
export const TeacherSchema = z.object({
    ...
    teacherindex: z.coerce
        .number({ message: "Teacher index must be a number" })
        .int()
        .min(0, { message: "Teacher index must be positive" })
        .optional(),
    classtypeid: z.coerce
        .number({ message: "Class type ID must be a number" })
        .int()
        .min(0, { message: "Class type ID must be positive" })
        .default(1),
