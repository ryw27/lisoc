...
export async function getSelectOptions() {
    ...
    const idMaps = {
        teacherMap: Object.fromEntries(teachers.map((teacher) => [teacher.teacherid, teacher])),
        classMap: Object.fromEntries(classes.map((cls) => [cls.classid, cls])),
        roomMap: Object.fromEntries(rooms.map((room) => [room.roomid, room])),
        timeMap: Object.fromEntries(times.map((time) => [time.timeid, time])),
        termMap: Object.fromEntries(terms.map((term) => [term.termno, term])),
    } satisfies IdMaps;
