    ...
    return await client.transaction(async (tx) => {
        ...
        */
        ...
        const year = await tx.query.seasons.findFirst({
            where: (s, { eq }) =>
                and(
                    eq(s.relatedseasonid, active_season.beginseasonid),
                    eq(s.beginseasonid, active_season.beginseasonid)
                ),
            orderBy: (s, { asc }) => asc(s.seasonid),
        });
        ...
        const fall = active_season.isspring
        ...
        const spring = active_season.isspring
            ? active_season
            : await tx.query.seasons.findFirst({
                  where: (s, { eq }) =>
                      and(
