...
import { canRegister, ensureSeats, ensureTimeline, getArrSeason, getTotalPrice } from "../data";

// Validation failures are returned, not thrown: Next.js strips thrown Server Action messages in
// production builds, so the client would only ever see a generic error. Keep this envelope so the
// reason ("This class is full/注册已满", schedule conflict, ...) actually reaches the user.
export type RegisterResult =
    | { ok: true; balance: fambalanceObj }
...
export async function familyRegister(
    ...
    try {
        return await db.transaction(async (tx) => {
            ...
            // 4b. Ensure the class still has an open seat
            const hasSeat = await ensureSeats(tx, arrData, season.seasonid);
            if (!hasSeat) {
                throw new Error("This class is full/注册已满");
            }
        ...
        */
            ...
            return { ok: true as const, balance: balanceObj };
        });
    } catch (err) {
        console.error("familyRegister failed: ", err);
        return {
            ok: false as const,
            errorMessage:
