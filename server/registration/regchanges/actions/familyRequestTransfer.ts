    ...
    try {
        const txResult = await db.transaction(async (tx) => {
            ...
            if (!oldReg) {
                return {
                    ok: false,
                    message: "Did not find old class registration being transferred out of",
                };
            }
            ...
            if (
                ...
                oldReg.statusid !== REGSTATUS_REGISTERED
            ) {
            ...
            if (!newArrSeason) {
                return {
                    ok: false,
                    message: "No corresponding season for class being transferred into",
                };
            }
            try {
                if (
            ...
            await tx.insert(regchangerequest).values({
                regid: oldReg.regid,
