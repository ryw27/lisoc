...
export default async function RegistrationHistory() {
    ...
    const getNiceArrInfo = async (reg: InferSelectModel<typeof classregistration>) => {
        const arr = await db.query.arrangement.findFirst({
            where: (a, { and, or, eq }) => or(
                and(
                    eq(a.classid, reg.classid),
                    eq(a.seasonid, reg.seasonid),
                ),
        ...
        let halfsem = false;
        if (arrSeason.seasonid < arrSeason.beginseasonid && arrSeason.seasonid < arrSeason.relatedseasonid) {
            halfsem = true;
        }

        let totalPrice = 0;
        if (halfsem) {
            totalPrice = Number(arr.tuitionH || "0") + Number(arr.bookfeeH || "0") + Number(arr.specialfeeH || "0");
        } else {
            totalPrice = Number(arr.tuitionW || "0") + Number(arr.bookfeeW || "0") + Number(arr.specialfeeW || "0");
        }
    ...
    return (
        <Suspense fallback={<div className="text-center py-8">Loading registration history...</div>}>
