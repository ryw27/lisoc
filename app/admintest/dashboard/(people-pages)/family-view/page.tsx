import { getFamilies, familyColumns, Family, deleteFamilies } from '@/app/admintest/dashboard/(people-pages)/family-view/family-helpers'
...
export default async function FamiliesPage({ 
    ...
    const { families, totalCount } = await getFamilies(
        validPage,
        validPageSize,
        query
    );
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold">View Families</h1>
                <Link 
                    href="/admintest/dashboard/add-family" 
            ...
            <HorizontalNav
                ...
                activeHref={`/admintest/dashboard/family-view`}
            />
            <div className="flex items-center px-4 py-2 gap-1">
                <p className="text-lg font-bold">All families </p>
                <span className="text-sm text-gray-500">({totalCount})</span>
            </div>
            
            <DataTable 
                data={families as Family[]} 
                columns={familyColumns as FilterableColumn<Family>[]}
                totalCount={totalCount}
                tableName="Families"
                primaryKey="familyid"
                deleteAction={deleteFamilies}
            />
        </div>
    )
}