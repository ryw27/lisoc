...
export default async function TeacherPage() {
    ...
    const { year, fall, spring } = { year: activeYear.seasonid, fall: fall_season.seasonid, spring: spring_season.seasonid };

    const classesToTeach = await db.query.arrangement.findMany({
        where: (arrangement, { and,eq, or }) => and(eq(arrangement.teacherid, teacher.teacherid), or(eq(arrangement.seasonid, fall), eq(arrangement.seasonid, spring), eq(arrangement.seasonid, year))),
        with: {
            class: {
                columns: {
                    classnamecn: true,
        ...
        <div className="container mx-auto flex flex-col">
            {classesToTeach.map(async (c) => {
                    ...
                    const students = await db.query.classregistration.findMany({
                        ...
                        where: (reg, { and, eq,or }) =>
                            and(eq(reg.classid, c.classid), eq(reg.isdropspring, false),eq(reg.seasonid, c.seasonid), or(eq(reg.statusid, REGSTATUS_REGISTERED),eq(reg.statusid,REGSTATUS_SUBMITTED))),
                        with: {
                            student: {
                                columns: {
                                    namecn: true,
