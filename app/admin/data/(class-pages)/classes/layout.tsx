...
import { IdMaps } from "@/types/shared.types";
import { type ClassObject } from "@/server/data-view/entity-configs/(classes)/classes";
...
import { useIdMaps } from "@/components/data-view/providers";
...
export default function ClassLayoutPage({ children }: { children: React.ReactNode }) {
    ...
    return (
        <ConfigLayout entity="classes" columns={classColumns}>
            {children}
        </ConfigLayout>
    );
}
    ...
    const classColumns: FilterableColumn<ClassObject>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <span title={`Class Type ID: ${typeid}`}>{type?.typenamecn || "Unknown"}</span>
                );
            },
        },
        {
        ...
        },
        {
            ...
            meta: {
                filter: {
                    type: "date",
                    mode: ["in the last", "=", "between", ">=", "<="] as const,
                    options: ["hours", "days", "months", "years"] as const,
                },
                label: "Last Modified",
            },
            header: ({ column }) => (
