...
export async function familyRegister(
    ...
    await db.transaction(async (tx) => {
        ...
        if (existingBal) {
            ...
            const newyearclass4child = (existingBal.yearclass4child ? existingBal.yearclass4child + 1 : 1);

            const newChildNum = (existingBal.childnum ? existingBal.childnum + 1 : 1);
            const newTuition = (existingBal.tuition ? Number(existingBal.tuition) + classPrice : classPrice);
            const newTotal = (existingBal.totalamount ? Number(existingBal.totalamount) + classPrice : classPrice);
            await tx
