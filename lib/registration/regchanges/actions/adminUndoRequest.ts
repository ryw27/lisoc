...
export async function adminUndoRequest(requestid: number,familyid:number, status:number ) {
    ...
    try
    {
        await db.transaction(async (tx) => {
            ...
            if (status != orgReq.reqstatusid)  // status passed in must match in case some one changed in middle
            {
            ...
            if (status == REQUEST_STATUS_REJECTED)
            {
                ...
                // lock for update 
                    ...
                    await tx
                        ...
                        .where(and(eq(regchangerequest.requestid, requestid),
            ...
            else if (status == REQUEST_STATUS_APPROVED)
            {
                ...
                {   
                    ...
                    if(updatedReg.statusid !== REGSTATUS_DROPOUT && updatedReg.statusid !==REGSTATUS_DROPOUT_SPRING)
                    {
                        throw new Error(`registration ${requestid} status has been changed can not rollback ` )
                    }
                    ...
                    await tx
                        ...
                        .where(and(eq(classregistration.regid, updatedReg.regid),
                    ...
                    const [linkedBalance] = await tx
                                            .select()
                                            .from(familybalance)
                                            .where(and(or(eq(familybalance.balanceid, relatedBalanceId),
                    ...
                    {   //if it is 0 it will throw error drop must have balance linked  
                        throw new Error(`linked balance id relatedBalanceId, for requestid ${requestid} does not exist`)
                    }
                ...
                else 
                {
                    ...
                    {
                        throw new Error(`registration ${orgReq.regid} status has been changed can not rollback ` )
                    }
                    ...
                    await tx
                        ...
                        .where(and(eq(classregistration.regid, orgReq.appliedid || 0),  // should not be 0
                                    eq(classregistration.familyid, familyid ))) 
                        // delete classregistration of regid                                    
                    await tx
                        .delete(classregistration)
                        .where(and(eq(classregistration.regid, orgReq.regid),
                                    eq(classregistration.familyid, familyid ))) 
