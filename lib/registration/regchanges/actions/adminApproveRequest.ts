...
export async function adminApproveRequest(requestid: number, registerid: number, adminMemo: string, balanceType:number,extraFee:number ) {
    // TODO: Parse
    // 1. Auth check
            ...
            await db.transaction(async (tx) => {
                ...
                if (oldReg.statusid !== REGSTATUS_REGISTERED) {
                    /*await tx
                        ...
                        .where(eq(regchangerequest.requestid, requestid));*/
                    throw new Error(`request id = ${request}, regid = ${registerid} is not in registered status , someone else may have change it`)
                }
