...
export const studentFormSchema = z.object({
	familyid: z.coerce.number({ message: "Family ID must be a number"}).int().min(1, { message: "Family ID must be positive"}),
	...
	age: z.coerce.number({ message: "Age must be a number"}).int().min(0, { message: "Age must be positive"}),
	dob: z
