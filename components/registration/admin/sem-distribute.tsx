...
export default function BulkTransfer({
    ...
    idmaps: IdMaps;
    onClose: () => void;
    onUpdate: (newData: fullRegID) => void;
}) {
    ...
    const dest_items = students_in_dest.map((c, index) =>
        c.map((s) => ({
            source: index,
            id: s.studentid.toString(),
            label: s.namecn + " (" + s.namelasten + " " + s.namefirsten + ")",
    ...
    const [destinationBoxes, setDestinationBoxes] = useState<Item[][]>(
        Array(numOfDestClasses)
            .fill(null)
            .map((_, i) => dest_items[i] || [])
    );
    ...
    const handleDropToDestBox = (boxIndex: number) => {
        if (draggedItem) {
            ...
            if (source == -1) {
                ...
                const studentIndex = newData.classrooms[source].students.findIndex(
                    (s) => s.studentid.toString() === draggedItem.id
                );
                if (studentIndex !== -1) {
    ...
    const handleRollback = () => {
        const allItems = [...sourceItems, ...destinationBoxes.flat()];
    ...
    const performDbUpdate = async () => {
        ...
        try {
            ...
            const allStudents = [
                ...dataWithStudents.students,
                ...dataWithStudents.classrooms.flatMap((c) => c.students),
            ];
            const studentsMap = new Map(allStudents.map((s) => [s.studentid.toString(), s]));

            //  source
            const oldSourceIds = new Set(
                dataWithStudents.students.map((c) => c.studentid.toString())
            );

            const extraInSource = sourceItems.filter(
                (item) => !oldSourceIds.has(item.id.toString())
            );
            ...
            if (extraInSource.length > 0) {
                ...
                for (const item of extraInSource) {
                    ...
                    const newreg = await adminTransferStudent2(
                        theStudent.regid,
            ...
            for (let i = 0; i < classrooms.length; i++) {
                const oldDestIds = new Set(
                    dataWithStudents.classrooms[i].students.map((c) => c.studentid.toString())
                );

                const extraInDest = destinationBoxes[i].filter(
                    (item) => !oldDestIds.has(item.id.toString())
                );
            ...
            if (changed) {
                ...
                modifiedData.students = allStudents.filter((s) =>
                    sourceItems.some((item) => item.id === s.studentid.toString())
                );

                modifiedData.classrooms = modifiedData.classrooms.map((c, index) => ({
                    ...c,
                    students: allStudents.filter((s) => {
                        return destinationBoxes[index].some(
                            (item) => item.id === s.studentid.toString()
                        );
                    }),
                }));
    ...
    const generateMoved = () => {
        ...
        if (extraInSource.length > 0) {
            for (const item of extraInSource) {
                ...
                moved.push([
                    item.id,
                    studentsMap.get(item.id.toString())?.familyid,
                    studentsMap.get(item.id.toString())?.regid,
                    newArrangeObj,
                ]);
            }
        }
    ...
    return (
        <div className="p-8">
            <h1 className="text-1XL mb-2 justify-center text-center font-bold">
                Bulk Transfer (批量转课){" "}
            </h1>
            ...
            <div className="mb-8">
                <h2 className="mb-4 text-xl font-semibold">
                    {source_name}: {source_limit} / {sourceCount}
                </h2>
                <div
                    ...
                    <div className="grid grid-cols-4 gap-1">
                        {sourceItems.map((item) => (
                            <div
                                ...
                                className="cursor-move rounded bg-blue-500 p-2 text-xs text-white transition hover:bg-blue-600"
                            >
                                {item.label}
                            </div>
                        ))}
            ...
            <div className="mb-8 flex justify-center gap-4">
                <button
                    ...
                    className="rounded-lg bg-blue-600 px-6 py-2 font-semibold text-black transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-gray-400 disabled:text-gray-600"
                ...
                <button
                    ...
                    className="rounded-lg bg-red-600 px-6 py-2 font-semibold text-black transition hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-gray-400 disabled:text-gray-600"
            ...
            {showConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    />
                    <div className="relative w-[min(92%,560px)] rounded-lg bg-white px-6 py-6 shadow-lg">
                        <h3 className="mb-2 text-xl font-semibold">Confirm Update</h3>
                        <p className="mb-6 text-sm text-gray-600">
                            This will Update Database. Are you sure?
                        </p>
            ...
            >
                {destinationBoxes.map((items, boxIndex) => {
                    ...
                    return (
                        <div key={boxIndex}>
                            <h2 className="mb-4 text-xl font-semibold">
                                {dest_names[boxIndex]} {dest_limits[boxIndex]} /
                                {destCounts[boxIndex]}
                            </h2>
                            <div
                                onDragOver={handleDragOver}
                                onDrop={() => handleDropToDestBox(boxIndex)}
                                className={`${bgClass} border-2 ${borderClass} h-[200px] overflow-y-auto rounded-lg p-4`}
                            >
            ...
            <div className="mt-8 space-y-2 text-center text-sm text-gray-500"></div>
            {/* Action Buttons */}

            <div className="padding-12 mb-8 flex justify-end gap-2">
                <button
                    onClick={() => {
                        onClose();
                    }}
                    className="rounded-lg bg-blue-600 px-6 py-2 text-white hover:bg-red-300"
                >
                    Cancel
                </button>
                <button
                    ...
                    className="rounded-lg bg-red-600 px-6 py-2 text-white hover:bg-blue-300"
                >
                    Update
                </button>
            </div>
