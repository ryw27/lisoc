...
import { Duration, formatISO, isValid, parseISO, startOfToday, sub } from "date-fns";
import { ChevronDown, FilterIcon, PlusIcon } from "lucide-react";
...
// ------------------------------------------------------------
export type filterMode = "=" | "≠" | ">" | ">=" | "<" | "<=" | "between" | "in the last";
...
const modeToSuffix = (mode: filterMode): string | undefined => {
...
function validateEntry(
    ...
    switch (entry.mode) {
        ...
        case "in the last": {
            ...
            if (!entry.aux || !unitOptions.includes(entry.aux as (typeof unitOptions)[number]))
                return "Unit missing";
            return null;
        }
...
export default function Filter<TData>({ columns }: { columns: FilterableColumn<TData>[] }) {
    ...
    const dateFilterPresent = useMemo(
        () =>
            drafts.some(
                (d) => columns.find((c) => c.id === d.col_id)?.meta?.filter?.type === "date"
            ),
        [drafts, columns]
    );
    ...
    const errors: string[] = useMemo(() => {
        return drafts
            .map((d) =>
                validateEntry(
                    d,
                    columns.find((c) => c.id === d.col_id)?.meta?.filter?.type || "text"
                )
            )
    ...
    const handleApply = () => {
        ...
        drafts.forEach((filter) => {
            ...
            const key = modeToSuffix(filter.mode)
                ? `${filter.col_id}${modeToSuffix(filter.mode)}`
                : filter.col_id;
    ...
    return (
        <Popover>
            <PopoverTrigger
                className={cn(
                    "flex items-center gap-2 rounded-md border border-gray-300 bg-white px-2 py-1 text-sm text-gray-500",
                    "cursor-pointer transition-colors duration-200 hover:bg-blue-600 hover:text-white"
                )}
            >
                <FilterIcon className="h-4 w-4" /> Filters
            </PopoverTrigger>
            <PopoverContent
                className="relative mt-2 w-max rounded-md border border-gray-300 bg-white p-4 shadow-lg"
                align="start"
                alignOffset={0}
                side="bottom"
                sideOffset={5}
            >
                ...
                <div className="absolute -top-[10px] left-[10px] z-30 h-0 w-0 border-x-8 border-b-[10px] border-x-transparent border-b-white"></div>
                {/* Border for the arrow */}
                <div className="absolute -top-[11px] left-[10px] z-20 h-0 w-0 border-x-8 border-b-[10px] border-x-transparent border-b-gray-300"></div>
                    ...
                    <div className="mt-2 flex flex-col gap-2">
                        {drafts.map((filter) => (
                            <FilterBox
                                ...
                                dateExists={
                                    dateFilterPresent &&
                                    columns.find((c) => c.id === filter.col_id)?.meta?.filter
                                        ?.type !== "date"
                                }
                                onChange={(payload) =>
                                    dispatch({ type: "update", id: filter.id, payload })
                                }
                        ...
                        <div className="flex items-center justify-between">
                            <button
                                type="button"
                                className="flex cursor-pointer items-center gap-1 self-start text-sm text-blue-600 hover:text-blue-700"
                                onClick={() => {
