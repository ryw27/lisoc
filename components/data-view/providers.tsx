...
import { FilterableColumn, PKName, Table } from "@/types/dataview.types";
...
export function IdMapsProvider({ children, idMaps }: IdMapsProviderProps) {
    return <IdMapsContext.Provider value={idMaps}>{children}</IdMapsContext.Provider>;
}
...
interface DataEntityProviderProps<T extends Table, RowType> extends DataEntityContextValue<
    T,
    RowType
> {
    children: ReactNode;
...
export function DataEntityProvider<T extends Table, RowType>({
    ...
    return <DataEntityContext.Provider value={value}>{children}</DataEntityContext.Provider>;
}
